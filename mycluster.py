            ...
            try:
                if self.mergethisnode(name,old=1,move=1):
                    ...
                    try:
                        ...
                        self.logger.exception(f'could not move nodedir:{nodedir} to destination_nodedir:{destination_nodedir}')
                    ...
                    while not all([idx=='saved' for idx in save_idx]):
                        if not type(save_idx[0]) is str:
                            try:
                                shutil.move(os.path.join(self.masterdirectory, name + '.name'),)
                                save_idx[0]='saved'
                            except shutil.Error:
                                self.logger.exception(f'move failed for namefile, idx:{save_idx[0]}')
                                try:
        ...
        while waiting==0:
            ...
            try:
                ...
                if last_status=='ready':
                    ...
                    #print('myjob status:',myjob['status'])
                    i+=1
                    sleep(.25*log(10*i+1))
            except:
    ...
    def check_node_job_status(self,name,time=None):
        ...
        for i in range(3):
            try:
                ...
                #print(f'check_node_job_status found: nodes_jobdict["status"]:{nodesjob_dict["status"]}')
                if time==0:
                    return nodesjob_dict['status'][-1][1]
                if time==1:
                    #print(f"nodesjob_dict['status'][-1]:{nodesjob_dict['status'][-1]}")
